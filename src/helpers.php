...
function fluent(string $id, array $args = [], string $locale = null)
{
...
function fluent_html(string $html, string $id, array $args = [], string $locale = null)
{
    if ($bundle = fluent_bundle($id, $locale)) {
        $message = $bundle->getMessage($id);
...
function time_ago($then)
{
    ...
    return fluent('time-ago', $args);
}

function short_time_ago($then)
{
    $args = TimeAgo::calculate($then);

    return fluent('short-time-ago', $args);
}
...
function relative_time($then)
{
    ...
    return fluent('relative-time', $args);
}
...
function full_time($date)
{
    ...
    return fluent('full-time', [
        'date' => new \Tobyz\Fluent\Types\DateTime($date/*, ['timeZone' => 'Australia/Adelaide']*/)
    ]);
}

function highlight_words(string $string, ?string $search)
{
    if (! $search) {
        return $string;
    }
