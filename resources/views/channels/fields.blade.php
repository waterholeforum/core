...
<div>
    ...
    <input
        ...
        name="hide_sidebar"
        value="1"
        @if (old('hide_sidebar', $channel->hide_sidebar ?? false)) checked @endif
    >
    @error('hide_sidebar')
        <div>{{ $message }}</div>
    @enderror
...
<fieldset>
    ...
    @foreach (['list' => 'List', 'cards' => 'Cards'] as $key => $name)
        <div>
            <input
                type="radio"
                name="default_layout"
                ...
                value="{{ $key }}"
                @if (old('default_layout', $channel->default_layout ?? null) === $key) checked @endif
            >
            <label for="default_layout_{{ $key }}">{{ $name }}</label>
        </div>
    @endforeach
...
<fieldset>
    ...
    <input
        ...
        name="custom_sorts"
        value="1"
        @if (old('custom_sorts', $channel?->default_sort || $channel?->sorts)) checked @endif
    >
    ...
    @foreach (\Waterhole\Extend\FeedSort::getComponents()->map(fn($class) => app($class)) as $sort)
        ...
        <div>
            <input
                ...
                name="default_sort"
                value="{{ $sort->handle() }}"
                @if (old('default_sort', $channel->default_sort ?? config('waterhole.forum.default_sort')) === $sort->handle()) checked @endif
            >
            <input
                type="checkbox"
                id="sort_{{ $handle }}"
                name="sorts[]"
                value="{{ $sort->handle() }}"
                @if (in_array($handle, old('sorts', $channel->sorts ?? config('waterhole.forum.sorts', [])))) checked @endif
            >
